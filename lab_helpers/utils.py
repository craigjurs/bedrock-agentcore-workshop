...
username = "testuser"
sm_name = "redcross_chatbot"


role_name = f"RedCrossChatbotBedrockAgentCoreRole-{REGION}"
policy_name = f"RedCrossChatbotBedrockAgentCorePolicy-{REGION}"
...
def get_cognito_client_secret() -> str:
    ...
    response = client.describe_user_pool_client(
        UserPoolId=get_ssm_parameter("/app/redcross/agentcore/pool_id"),
        ClientId=get_ssm_parameter("/app/redcross/agentcore/client_id"),
    )
...
def save_redcross_chatbot_secret(secret_value):
    ...
    try:
        secrets_client.create_secret(
            ...
            Description="Secret containing the Cognito Configuration for the American Red Cross Chatbot",
        )
...
def get_redcross_chatbot_secret():
...
def delete_redcross_chatbot_secret():
...
def get_or_create_cognito_pool(refresh_token=False):
    ...
    try:
        ...
        cognito_config_str = get_redcross_chatbot_secret()
    ...
    try:
        ...
        put_ssm_parameter("/app/redcross/agentcore/client_id", client_id)
        put_ssm_parameter("/app/redcross/agentcore/pool_id", pool_id)
        put_ssm_parameter(
            "/app/redcross/agentcore/cognito_discovery_url", discovery_url
        )
        put_ssm_parameter("/app/redcross/agentcore/client_secret", client_secret)

        save_redcross_chatbot_secret(json.dumps(cognito_config))
...
def create_agentcore_runtime_execution_role():
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    ...
                    f"arn:aws:bedrock-agentcore:{region}:{account_id}:workload-identity-directory/default/workload-identity/redcross_chatbot-*",
                ],
            },
            {
    ...
    try:
        ...
        put_ssm_parameter(
            "/app/redcross/agentcore/runtime_execution_role_arn",
...
def runtime_resource_cleanup(runtime_arn: str = None):
    try:
        ...
        for repo in repositories["repositories"]:
            if "bedrock-agentcore-redcross_chatbot" in repo["repositoryName"]:
...
def delete_observability_resources():
    # Configuration
    log_group_name = "agents/redcross-chatbot-logs"
